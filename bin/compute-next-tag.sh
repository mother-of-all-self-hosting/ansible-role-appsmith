...
#
# Tags look like `v<Appsmith version>-<release>`:
#
# - if defaults/main.yml points at an Appsmith version that has never been
#   released, the release counter restarts at 0 (`v2.3-0`)
# - otherwise the counter is incremented (`v2.3-1`), but only if something
...
version="$(sed -nE 's|^appsmith_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Appsmith version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v2.3`),
# which must not be doubled in the tag.
#
# Appsmith numbers its releases `v<major>.<minor>` these days, and used to
# number them `v<major>.<minor>.<patch>` - occasionally with a fourth
# component (`v1.9.37.1`). None of that is interpreted here: the version is
# carried into the tag verbatim, so every shape releases the same way.
...
#
# The trailing dash in the pattern matters more here than it does elsewhere in
# the fleet: Appsmith went from `v1.9` to `v1.90`, so a prefix match without it
# would mistake one line of releases for another.
